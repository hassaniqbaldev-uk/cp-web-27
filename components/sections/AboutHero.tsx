...
import MarqueeDrag from "../ui/MarqueeDrag";
      ...
      >
        <Container className="relative">
          ...
          <div className="max-425:overflow-hidden max-425:motion-reduce:overflow-x-auto max-425:mt-[29rem] relative z-[12] mt-[14rem]">
            {/* Holds the track while a finger is down and lets it be dragged.
                Its only child must be the track, which is what it measures. */}
            <MarqueeDrag>
              {/* A list, so it announces as four items rather than eight loose
                  strings. Paused by --play while dragging, and on hover for
                  anyone reading on a desktop at this width. */}
              <ul className="max-425:animate-marquee-left max-425:w-max max-425:flex-nowrap max-425:[--marquee-gap:0.6rem] max-425:flex max-425:[animation-play-state:var(--play,running)] max-425:hover:[animation-play-state:paused] grid grid-cols-4 gap-[.6rem] motion-reduce:animate-none">
